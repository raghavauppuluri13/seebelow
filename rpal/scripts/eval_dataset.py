...
from rpal.utils.pcd_utils import (scan2mesh, mesh2roi, visualize_pcds, color_icp, color_filter,
                                  mesh2polyroi, pick_surface_bbox, stl_to_pcd, color_entity,
                                  disk_pcd, clustering, inverse_crop)
...
EVAL_CROP = {
    'hemisphere':
    np.array([[0.5325745344161987, 0.005577473901212215, 0.0685637779533863],
    ...
    'crescent':
    np.array([[0.5196736454963684, -0.03556692600250244, 0.06781825795769691],
...
if __name__ == "__main__":
    ...
    gt_tumors_scan_pcd = [color_entity(gt_tumor, color_map='rainbow') for gt_tumor in gt_tumors]
    gt_tumors_scan_mesh = [color_entity(scan2mesh(gt_tumor)) for gt_tumor in gt_tumors_scan_pcd]
    dataset_map = {}
    ...
    for i, dataset_path in enumerate(datasets):
        ...
        tumor_mesh_without_CF = mesh2polyroi(color_entity(scan2mesh(tumor_pcd_without_CF)),
                                             polybox_pts=EVAL_CROP[tumor_type],
                                             return_mesh=True)
        tumor_mesh_with_CF = mesh2polyroi(color_entity(scan2mesh(tumor_pcd_with_CF)),
                                          polybox_pts=EVAL_CROP[tumor_type],
                                          return_mesh=True)
        ...
        if args.glob:
            ...
            print('current # of samples for this category: ',
    ...
    if args.combine:
        ...
        for tumor in TUMOR_ID.keys():
            for algo in ALGOS:
                ...
                print("MEAN {},{} #{}: {}".format(tumor, algo, len(fscores_ids),
                                                  np.mean(f_scores[fscores_ids], axis=0)))
                print("MAX {},{} #{}: {}".format(tumor, algo, len(fscores_ids),
                                                 np.max(f_scores[fscores_ids], axis=0)))
                print("MIN {},{} #{}: {}".format(tumor, algo, len(fscores_ids),
