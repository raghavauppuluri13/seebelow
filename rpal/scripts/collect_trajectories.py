...
#from devices import RealsenseCapture, KinectCapture

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--interface-cfg", type=str, default="pan-pan.yml")
    parser.add_argument(
...
def main():
    ...
    dataset_folder = datetime.datetime.now().strftime("{}/dataset_%m-%d-%Y_%H-%M-%S".format(data_dir.absolute())
    ...
    np.savez(
        f"{folder}/proprio_joints", data=np.array(data["proprio_joints"])
    )
