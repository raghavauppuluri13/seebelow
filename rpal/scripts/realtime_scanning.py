...
if __name__ == "__main__":
    ...
    argparser.add_argument("--calibration-cfg", type=str, default="pan-pan-force.yml")
    ...
    with open(str(RPAL_CFG_PATH.parent / 'temp_config' / args.calibration_cfg / "extrinsics.yaml"), "r") as file:
    ...
    try:
        while 1:
            ...
            O_T_C = O_T_EE @ np.linalg.inv(C_T_E)
            new_pcd.transform(O_T_C)

            pcd.points = new_pcd.points
            pcd.colors = new_pcd.colors

            new_f = o3d.geometry.TriangleMesh.create_coordinate_frame(
                size=0.1  # specify the size of coordinate frame
            )
            new_f.transform(O_T_C)

            f.triangles = new_f.triangles
            f.vertices = new_f.vertices
            f.vertex_normals = new_f.vertex_normals
            f.triangle_normals = new_f.triangle_normals
